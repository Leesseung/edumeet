  ...
  <div v-if="isVisible" class="modal-overlay" @click="closeModal">
    <div class="modal-container" @click.stop>
      ...
      <div v-if="isMyCreatedClass && assignmentData.studentSubmissionStatuses?.length" class="student-submission-status section">
        ...
        <div class="section-content">
          <ul>
            <li v-for="student in assignmentData.studentSubmissionStatuses" :key="student.studentEmail">
              <span class="student-name">{{ student.studentName }}</span>:
              <span :class="{'submitted': student.status !== 'NOT_SUBMITTED', 'not-submitted': student.status === 'NOT_SUBMITTED'}">
              ...
              <!-- 제출 파일 -->
              <ul v-if="student.submissionFiles?.length" class="submission-files">
                <li v-for="file in student.submissionFiles" :key="file.uuid">
                  <template v-if="isImage(file.fileName)">
                    <button class="file-btn image-btn" @click="openImageModal(file.s3Url)">
                      {{ file.fileName }}
                    </button>
      ...
      <!-- 학생 제출 확인 (학생용, 제출 완료) -->
      <div v-if="!isMyCreatedClass && assignmentData.done" class="student-submission-status section">
        <div class="section-title-box">
          <h4 class="section-title">제출 파일</h4>
        </div>
        <div class="section-content">
          <ul class="submission-files">
            <li v-for="file in assignmentData.studentSubmissionFiles" :key="file.uuid">
              <template v-if="isImage(file.fileName)">
                <button class="file-btn image-btn" @click="openImageModal(file.s3Url)">
                  {{ file.fileName }}
                </button>
...
.section { padding: 1.5rem; background-color: #1a202c; border-bottom: 1px solid #334155; max-height: 200px; overflow-y: hidden; transition: all 0.3s ease; }
.section:hover { overflow-y: auto; }
.section:last-of-type { border-bottom: none; }
.section-title-box { background-color: #2d3748; padding: 0.75rem 1.5rem; border-radius: 8px; margin-bottom: 1rem; width: fit-content; }
.section-title { margin: 0; font-weight: bold; color: #f1f5f9; font-size: 1.2rem; }
.section-content p { margin: 0; }
.attachment-files ul, .student-submission-status ul, .submission-files { list-style: none; padding-left: 0; }
.attachment-files li, .submission-files li { margin-bottom: 0.5rem; display: flex; align-items: center; }
.file-btn, .file-link { color: #cbd5e1; text-decoration: underline; background: none; border: none; cursor: pointer; display: flex; align-items: center; }
.file-btn::before, .file-link::before { content: '📄'; margin-right: 8px; color: #90cdf4; }
.image-btn::before { content: '🖼️'; color: #f6ad55; }
.submitted { color: #22c55e; font-weight: bold; }
.not-submitted { color: #ef4444; font-weight: bold; }
.form-input { width: 100%; padding: 0.5rem; margin-top: 0.25rem; background-color: #4a5568; border: 1px solid #4a5568; color: #e2e8f0; }
.submit-btn { background-color: #3b82f6; color: #fff; border: none; padding: 0.75rem 1.5rem; border-radius: 4px; cursor: pointer; width: 100%; font-weight: bold; margin-top: 1rem; }
.submit-btn:hover { background-color: #2563eb; }
.section::-webkit-scrollbar { width: 8px; }
.section::-webkit-scrollbar-thumb { background-color: #4a5568; border-radius: 4px; }
.section::-webkit-scrollbar-track { background-color: #2d3748; }
</style>
